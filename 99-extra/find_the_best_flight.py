...
def get_flight_minutes(departure, arrival):
    (departure_hours, departure_minutes) = map(int, departure.split(':'))
    (arrival_hours, arrival_minutes) = map(int, arrival.split(':'))
    return (arrival_minutes - departure_minutes) + ((arrival_hours - departure_hours) * 60)


def create_flight_graph(flights):
    graph = defaultdict(lambda: defaultdict(lambda: {}))
    flight_info = defaultdict(lambda: {})
    for (flight_num, origin, destin, departure, arrival, cost) in flights:
        graph[origin][destin][flight_num] = True
        flight_info[flight_num]['departure'] = departure
        flight_info[flight_num]['arrival'] = arrival
        flight_info[flight_num]['flight_minutes'] = get_flight_minutes(departure, arrival)
        flight_info[flight_num]['cost'] = cost
    return (graph, flight_info)


def is_better_flight(best_flights, flight_info, curr, neighbor):
    new_cost = best_flights[curr]['cost'] + flight_info[neighbor[1]]['cost']
    new_duration = (
        best_flights[curr]['flight_minutes'] +
        get_flight_minutes(flight_info[curr[1]]['arrival'], flight_info[neighbor[1]]['departure']) +
        flight_info[neighbor[1]]['flight_minutes']
    )
    if new_cost < best_flights[neighbor]['cost']:
        return True
    if new_cost == best_flights[neighbor]['cost'] and new_duration < best_flights[neighbor]['flight_minutes']:
        return True
    return False


def flight_dijkstra(graph, flight_info, start):
    ...
    path = { start_node: [] }
    while queue:
        curr = (curr_city, prev_flight) = queue.pop(0)
        for (next_city, flights) in graph[curr_city].items():
            for flight_num in flights.keys():
                # departure time is before previous flight's arrival time
                if prev_flight is not None and flight_info[flight_num]['departure'] < flight_info[prev_flight]['arrival']:
                    continue
                neighbor = (next_city, flight_num)
                if prev_flight is None:
                    best_flights[neighbor] = {
                        'cost': flight_info[neighbor[1]]['cost'],
                        'flight_minutes': flight_info[neighbor[1]]['flight_minutes'],
                    }
                    path[neighbor] = path[curr] + [neighbor]
                    queue.append(neighbor)
                elif neighbor not in best_flights or is_better_flight(best_flights, flight_info, curr, neighbor):
                    best_flights[neighbor] = {
                        'cost': best_flights[curr]['cost'] + flight_info[flight_num]['cost'],
                        'flight_minutes': (
                            best_flights[curr]['flight_minutes'] +
                            get_flight_minutes(flight_info[prev_flight]['arrival'], flight_info[flight_num]['departure']) +
                            flight_info[flight_num]['flight_minutes']
                        ),
                    }
                    path[neighbor] = path[curr] + [neighbor]
                    queue.append(neighbor)
    return (best_flights, path)


def find_best_flights(flights, origin, destination):
    (graph, flight_info) = create_flight_graph(flights)
    (best_flights, path) = flight_dijkstra(graph, flight_info, origin)
    result = None
    curr_best = None
    for (city, flight) in path.keys():
        if city == destination:
            if result is None:
                result = [path_flight for (_, path_flight) in path[(city, flight)]]
                curr_best = best_flights[(city, flight)]
            elif (best_flights[(city, flight)]['cost'] < curr_best['cost'] or
              (best_flights[(city, flight)]['cost'] == curr_best['cost'] and
               best_flights[(city, flight)]['flight_minutes'] < curr_best['flight_minutes'])):
                result = [path_flight for (_, path_flight) in path[(city, flight)]]
                curr_best = best_flights[(city, flight)]
            print origin, '=>', destination
            print path[(city, flight)]
            print best_flights[(city, flight)]
            print '######################################'
    print 'result:', result
