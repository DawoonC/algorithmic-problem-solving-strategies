...
def maximize_probability_of_favor(G, v1, v2):
    ...
    edge_cnt = 0
    node_cnt = 0
    for v in inverted.keys():
        node_cnt += 1.0
        for vv in inverted[v].keys():
            inverted[v][vv] = -log(inverted[v][vv], 2)
            edge_cnt += 1.0
    dijkstra = dijkstra_heap if (edge_cnt / node_cnt) < 50 else dijkstra_list
    result = dijkstra(inverted, v1)
    path = []
    prob = 1.0
    queue = [v2]
    while queue:
